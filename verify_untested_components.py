...
from ibis.intelligence.market_intel import MarketIntelligence, MarketContext, MarketInsight, AnalysisDimension
from ibis.exchange.kucoin_client import KuCoinClient, Ticker, OrderBook, Candle
from ibis.intelligence.enhanced_sniping import EnhancedSnipingSystem
...
from ibis.pnl_tracker import PNLTacker
from ibis.state_sync import StateSync
from ibis.cross_exchange_monitor import CrossExchangeMonitor
from ibis.position_rotation import PositionRotation
from ibis.microstructure import MarketMicrostructure
...
async def test_market_intelligence():
    ...
    try:
        ...
        print(f"   Dimension: {AnalysisDimension(insight.dimension).value}")
        ...
        # Simulate conditions
        conditions = condition_detector.assess_conditions({
            "BTC-USDT": context
        })
        
        print(f"\n✅ Market conditions assessment")
        print(f"   Trend strength: {conditions['trend_strength']:.2f}")
        print(f"   Volatility risk: {conditions['volatility_risk']:.2f}")
        print(f"   Sentiment: {conditions['sentiment']:.2f}")
        print(f"   Liquidity: {conditions['liquidity']:.2f}")
