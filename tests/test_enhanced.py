...
def test_dynamic_stops(enhanced: IBISEnhancedIntegration):
    ...
    try:
        ...
        for regime, confidence, label in test_cases:
            ...
            print_result("Stop Loss:", f"${stops['stop_loss']:.2f}", f" (-{stops['stop_distance_pct']:.2f}%)")
            print_result("Take Profit:", f"${stops['take_profit']:.2f}", f" (+{stops['tp_distance_pct']:.2f}%)")
...
def test_risk_management(enhanced: IBISEnhancedIntegration):
    ...
    try:
        ...
        for balance in balances:
            ...
            print(f"         Reduce Risk: {'✅ Yes' if risk_check['should_reduce_risk'] else '❌ No'}")
            print(f"         Stop Trading: {'🛑 Yes' if risk_check['should_stop_trading'] else '✅ No'}")
...
async def run_all_tests():
    ...
    results.append(("AGI Integration", await test_agi_integration(enhanced, test_symbol, market_data)))
    ...
    results.append(("Comprehensive Analysis", await test_comprehensive_analysis(enhanced, test_symbol)))
